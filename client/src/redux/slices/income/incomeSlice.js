...
export const createIncomeAction = createAsyncThunk('income/create', async (payload, {rejectWithValue, getState, dispatch}) => {
    ...
    try {
        // Make HTTP call
        const {data} = await axios.post(`income/`, payload, config) /* incomeBaseURL + '/', payload */
...
export const fetchAllIncome = createAsyncThunk('income/fetch', async (payload, {rejectWithValue, getState, dispatch}) => {
    ...
    try {
        // Make HTTP call
        const {data} = await axios.get(`income/?page=${payload}`, config) /* incomeBaseURL + `/?page=${payload}` */
        return data
    } catch (error) {
        if(!error?.response) {
            throw error
...
export const updateIncomeAction = createAsyncThunk('income/update', async (payload, {rejectWithValue, getState, dispatch}) => {
    ...
    try {
        // Make HTTP call
        const {data} = await axios.put(`income/${payload?.id}`, payload, config) /* incomeBaseURL + `/${payload?.id}` */
