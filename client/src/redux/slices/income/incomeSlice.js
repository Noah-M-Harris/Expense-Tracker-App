...
export const createIncomeAction = createAsyncThunk('income/create', async (payload, {rejectWithValue, getState, dispatch}) => {
    ...
    try {
        // Make HTTP call
        const {data} = await axios.post(`/v1/income/`, payload, config) /* incomeBaseURL + '/', payload */
...
export const fetchAllIncome = createAsyncThunk('income/fetch', async (payload, {rejectWithValue, getState, dispatch}) => {
    ...
    try {
        // Make HTTP call
        const {data} = await axios.get(`/v1/income/?page=${payload}`, config) /* incomeBaseURL + `/?page=${payload}` */
        return data
    } catch (error) {
        if(!error?.response) {
            throw error
...
export const updateIncomeAction = createAsyncThunk('income/update', async (payload, {rejectWithValue, getState, dispatch}) => {
    ...
    try {
        // Make HTTP call
        const {data} = await axios.put(`/v1/income/${payload?.id}`, payload, config) /* incomeBaseURL + `/${payload?.id}` */
